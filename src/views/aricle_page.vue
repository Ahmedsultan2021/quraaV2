<template>
  <navbar />
<div class="container-fluid" style="background-image: linear-gradient(to bottom, #FECE2F15, #ffffff);" >
  <div class="container  " v-if="article" >
    <div class="row1 pt-5">
      <h1 class="full_article_title">{{ article.article_title }}</h1>
      <p>
        {{ article.article_from }}.{{ article.article_time_read }} الكاتب:
    ...
    <div class="row">
      <p class="full_article">
        {{ article.main_article }}
      </p>
      <div
        class="full_article"
        v-for="subArticle in article.full_article"
        :key="subArticle.sub_title_p"
      >
        <h3>{{ subArticle.sub_title_p }}</h3>
        {{ subArticle.p }}
      </div>
    </div>
    <hr />
    <div v-if="let_note_show">
      <div class="row">
        <textarea
          name="let_notes"
          id="let_notes"
          cols="30"
          rows="10"
          placeholder="اكتب ملحوظه  ......"
        ></textarea>
      </div>
      <div class="row">
        <div class="col v_button_col">
          <button class="v_button" v-if="button_appear">نشر</button>
        </div>
        <div class="col v_button_col">
          <button class="v_button" v-if="button_appear">حزف</button>
        </div>
      </div>
    </div>
    <div class="row see_note_row" v-if="see_note">
      <div class="see_note">
        {{ article.note }}
      </div>
    </div>
  ...
  <br />
  <div class="container2" v-if="comment">
    <div v-if="let_comment_show">
      ...
      <div class="row let_comment">
        <div class="col">
          <p>: اترك تعليق</p>
          <form action="">
            <input type="text" name="الاسم" placeholder="الاسم" id="" />
            <input
              type="email"
              name="البريد الالكترونى"
              placeholder="البرىد الالكتروني"
              id=""
            />
            <textarea
              class="text2"
              ...
              rows="10"
            ></textarea>
            <button class="send_comment">ارسال</button>
          </form>
        </div>
      </div>
    </div>

    <div v-if="comments_show" class="container comment">
      <div
        class="row"
        v-for="user_comment in comment.users_comments"
        :key="user_comment.user_name"
      >
        <div class="clinte">
          {{ user_comment.user_name }}
          <img src="../../public/img1.png"  class="rounded-circle" />
        </div>
        <div class="clinte_comment py-3">
          <p class="fs-6" >{{ user_comment.user_comment }}</p>
        </div>
      </div>
    </div>
  </div>
  ...
  <hr />
  <div class="container3 m-5" v-if="article_have_relations">
    <div class="row">
      <p>اقرا المزيد</p>
      <h1>مواضيع زات صله</h1>
    </div>
    <div class="row relation_article_row">
      <div
        class="col-4 position-relative"
        v-for="(article_have_relation,index) in article_have_relations" :key="index"
      >
        <div class="div_img1">
          <div>
            <!-- <img :src="article_have_relation.img_url"  class="img1"/> -->
            <img src="../../public/img1.png" class="img1" height="370" alt="" />
            <div class="img_detalis">
              <div class="p_time">
                {{ article_have_relation.from_time }} .
                {{ article_have_relation.time_to_raed }}
              </div>
              <p class="writer">الكاتب:{{ article_have_relation.write_by }}</p>
            </div>
            <h1 class="img1_titel1">{{ article_have_relation.titel }}</h1>
          </div>
        </div>
      </div>
    </div>
  ...
  mounted() {
    fetch("http://localhost:3000/articles/" + this.id)
      ...
      .then((data) => {
        ...
        const note = this.article.note;
        if (
          this.$store.state.user.role == "qpresedent" ||
          this.$store.state.user.role == "newspapereditor" ||
          this.$store.state.user.role == "author"
        ) {
          console.log(status);
        ...
        if (status == "pending" ) {
          ...
          this.let_comment_show = false;
          // this.let_note_show = true;
    ...
    // fetch("http://localhost:3000/articles/full_article")
    //   .then((res) => res.json())
    //   .then((data) => (this.full_article = data))
